...
public abstract class ValuesBuiltin extends ErlBuiltinNode {

    public ValuesBuiltin() {
        super(SourceSection.createUnavailable("Erlang builtin", "values"));
    }

    @Override
    public MFA[] getNames() {
        return new MFA[]{new MFA("maps", "values", 1)};
    }

    @Specialization
    public ErlList values(ErlMap map) {
        return ErlList.fromArray(map.getValueArray());
    }

    @Specialization
    public ErlList values(Object arg) {

        if (arg instanceof ErlMap) {
            return values((ErlMap) arg);
        }
