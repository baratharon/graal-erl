 ...
 */
@NodeInfo(shortName = "toList")
public abstract class ToListBuiltin extends ErlBuiltinNode {

    public ToListBuiltin() {
        super(SourceSection.createUnavailable("Erlang builtin", "to_list"));
    }
    ...
    @Specialization
    public ErlList toList(ErlMap map) {

        final Object[] keys = map.getKeyArray();
        final Object[] vals = map.getValueArray();
        ...
        for (int i = keys.length - 1; i >= 0; --i) {
            result = new ErlList(new ErlTuple(keys[i], vals[i]), result);
        }
    ...
    @Specialization
    public ErlList toList(Object arg) {

        if (arg instanceof ErlMap) {
            return toList((ErlMap) arg);
        }
