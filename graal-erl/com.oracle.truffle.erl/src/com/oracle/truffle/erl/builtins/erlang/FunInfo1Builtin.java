...
public abstract class FunInfo1Builtin extends ErlBuiltinNode {

    public FunInfo1Builtin() {
        super(SourceSection.createUnavailable("Erlang builtin", "fun_info"));
    }

    @Override
    public MFA getName() {
        return new MFA("erlang", "fun_info", 1);
    }

    @Specialization
    public ErlList funInfo(ErlFunction fun) {

        ErlList result = ErlList.NIL;

        for (FunInfoItem item : FunInfoItem.values()) {

            final ErlTuple tuple = fun.getInfo(item);
    ...
    @Specialization
    public ErlList funInfo(Object arg1) {
        return funInfo(ErlFunction.fromObject(arg1));
    }
}
