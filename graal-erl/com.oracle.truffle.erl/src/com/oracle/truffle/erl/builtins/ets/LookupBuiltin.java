...
public abstract class LookupBuiltin extends ErlBuiltinNode {

    public LookupBuiltin() {
        super(SourceSection.createUnavailable("Erlang ETS builtin", "lookup"));
    }
    ...
    @Specialization
    public ErlList lookup(Object tid, Object key) {
        final ErlTable tab = ErlTable.findTable(tid);
        if (null != tab) {
            return ErlList.fromIterator(tab.lookup(key).iterator());
        }
