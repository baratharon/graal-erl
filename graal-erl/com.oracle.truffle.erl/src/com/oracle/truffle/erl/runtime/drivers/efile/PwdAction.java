...
public class PwdAction extends AsyncActionSingle {
    ...
    public Result async() {
        ...
        final String str = Paths.get("").toAbsolutePath().toString();
        final ByteBuffer bb = ErlContext.UTF8_CHARSET.encode(str);
        final byte[] bytes = bb.array();
        final ErlBinary binary = ErlBinary.fromArray(Arrays.copyOf(bytes, bb.limit()));

        addResult(new ErlList((long) Driver.FILE_RESP_FNAME, binary));
        return Result.DONE;
    }
}
