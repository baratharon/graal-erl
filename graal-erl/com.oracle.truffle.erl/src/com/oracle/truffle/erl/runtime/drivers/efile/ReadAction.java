...
public class ReadAction extends AsyncActionSingle {

    private final FDFile file;
    private final long size;
    private ByteBuffer bin = null;

    public ReadAction(FDFile file, long size) {
        super();
        this.file = file;
        this.size = size;
    }
    ...
    public Result async() {
        ...
        if (null == bin) {
            ...
            if (size > Integer.MAX_VALUE || size < 0 || !file.canRead()) {
            ...
            try {
                ...
                addResult(new ErlList((long) Driver.FILE_RESP_ERROR, Driver.ENOMEM));
                return Result.ERROR;
            }
        }
        ...
        addResult(new ErlList((long) Driver.FILE_RESP_ALL_DATA, ErlBinary.fromArray(bytes)));
