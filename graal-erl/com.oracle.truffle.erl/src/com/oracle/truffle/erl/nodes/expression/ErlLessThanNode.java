 ...
 */
@NodeInfo(shortName = "<")
public abstract class ErlLessThanNode extends ErlBinaryNode {

    public ErlLessThanNode(SourceSection src) {
        super(src);
    }
    ...
    @Specialization
    protected boolean lessThan(BigInteger left, double right) {
        return left.doubleValue() < right;
    }

    @Specialization
    protected boolean lessThan(double left, BigInteger right) {
    ...
    @Specialization
    protected boolean lessThan(ErlAtom left, ErlAtom right) {
        return left.getValue().compareTo(right.getValue()) < 0;
    }

    @Specialization
    protected boolean lessThan(ErlTuple left, ErlTuple right) {
        return left.compare(right, false) < 0;
    }

    @Specialization
    protected boolean lessThan(ErlList left, ErlList right) {
        return left.compare(right, false) < 0;
    }

    @Fallback
    protected boolean lessThan(Object left, Object right) {
        ...
        return ErlContext.compareTerms(left, right, false) < 0;
    }
}
